import { chatHrefConstruction, cn } from "@/lib/utils";
...
const UnseenChatToast: FC<IUnseenChatToastProps> = ({ t, sessionId, senderId, senderImg, senderName, senderMessage }) => {
    return (
        <div
            className={cn("max-w-md w-full bg-white shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5", {
                "animate-enter": t.visible,
                "anumate-leave": !t.visible,
            })}
        >
            <a
                onClick={() => toast.dismiss(t.id)}
                href={`/dashboard/chat/${chatHrefConstruction(sessionId, senderId)}`}
